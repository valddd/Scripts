...
function SaveSettings()
    ...
    if not isfolder("VHUB") then
        makefolder("VHUB")
    end
    writefile("VHUB/" .. SaveFileName, HttpService:JSONEncode(TableToSave(Settings)))
end
...
function ReadSetting() 
    local s,e = pcall(function() 
        ...
        return HttpService:JSONDecode(readfile("VHUB/" .. SaveFileName))
    end)
    if s then return e 
    else
        SaveSettings()
    ...
    if tostring(Self)=="CIcklcon" then 
        ...
        if Settings.RifleKick then
            if GetVaoDau()>480 then 
                plr:Kick("\n[VHUB]\nKick 499ème coup de fusil (Fusil Kick)")
            end
        end
    end
    return old(...)
...
local VHub = 
loadstring(game:HttpGet("https://raw.githubusercontent.com/valddd/Scripts/main/rac"))()
local txt="GPO"
local Window = VHub:CreateWindow("<font color=\"#DA0317\">VHUB</font>", txt, true)
...
local lf = Section2:CreateToggle("Level Farm", {Toggled=Settings.Farm,Description = "Farm sur l'île FishMan (meilleur option)"}, function(state)
    Settings.Farm = state
...
Section2:CreateDropdown("Méthode Level Farm", {
...
local Section2 = Tab2:CreateSection("Autres Farm")
local AFB= Section2:CreateToggle("Auto Farm Beli (Débutant Uniquement)", {Toggled=Settings.Chest,Description = "Auto Farm les coffres puis les quêtes de Sarah"}, function(state)
...
Section2:CreateDropdown("Méthode Ship Farm", {
...
Section2:CreateButton("Set Position Bâteau", function(args)
    ...
    VHub:AddNoti("Notification", "Point de spawn des bâteaux correctement placés", 50)
end)
function TpE(e,f)
            ...
            if Settings.WebHookUrl then 
                local msg = {
                    ...
                    ["embeds"] =  {{
                        ...
                        ["description"] = "Fruit Droppé",
                        ["type"] = "rich",
                        ["color"] = tonumber(14287639),
                        ["fields"] = {
                            {
                                ["name"] = "Nom",
                                ["value"] = "||" .. plr.Name .. "||",
                        ...
                        ["image"] = {
                            ['url'] = "https://cdn.discordapp.com/attachments/1024646091079168071/1034492778182361209/standard_1.gif"
                        },
                        ["thumbnail"] = {
                            ["url"] = "https://cdn.discordapp.com/attachments/1024646091079168071/1034491801354121368/unknown.png"
                        },
                        ["footer"] = {
                            ["icon_url"] = "https://cdn.discordapp.com/attachments/1024646091079168071/1034490036764295188/standard.gif",
                            ["text"] = "VHub (" .. os.date("%X") .. ")"
                        }
                    }}
                }
...
Section2:CreateDropdown("Auto Buso Method", {
    List = {"Toujours actif","Actif Mob Proximité"},
    Default = Settings.BusoMethod or "Actif Mob Proximité"
}, function(item)
...
function SafeModeKick() 
    plr:Kick("\n[VHUB]\nKick pour mouvements suspects (Safe Mode)")
end
...
local Section2 = Tab2:CreateSection("Sécurité")
Section2:CreateToggle("Safe Mode", {Toggled=Settings.SafeMode,Description = "Vous serez kick si vous émettez des mouvements suspects"}, function(state)
    Settings.SafeMode = state
end)
Section2:CreateToggle("Auto Kick au 499ème tire", {Toggled=Settings.RifleKick,Description = "Vous serez kick si vous tirez plus de 499 fois au fusil"}, function(state)
    Settings.RifleKick = state
end)
Section2:CreateToggle("Auto Kick", {Toggled=Settings.AutoKick,Description = "Auto Kick après un temps défini"}, function(state)
            ...
            if Settings.AutoKickTimer then 
                pcall(function() 
                    ...
                    if Settings.AutoKick then
                        plr:Kick("\n[VHUB]\n".. Settings.AutoKickTimer .." minute atteint (Auto Kick)")
                    end
                end)
            end 
        end
...
Section2:CreateTextbox("PS Code", "(Laisser vide pour rejoindre un serveur normal)", function(args)
    Settings.PSCode=args
end,Settings.PSCode)
local Section2 = Tab2:CreateSection("Joueur")
Section2:CreateToggle("No Drown", {Toggled=Settings.Nodrown,Description = "Pas de noyade"}, function(state)
    Settings.Nodrown = state
    SetEN("Nodrown", "Setting", state)
end)
Section2:CreateToggle("No Fall", {Toggled=Settings.NoFallDame,Description = "Aucun dégats de chute"}, function(state)
    Settings.NoFallDame = state
    SetEN("NoFallDame", "Setting", state)
end)
Section2:CreateToggle("Infinite Dash", {Toggled=Settings.DashNoStam,Description = "Dashing sans perte d'endurance"}, function(state)
...
-- end)
local ws = 16
Section2:CreateSlider("Speed Changer", {Min = 16, Max = 100, DefaultValue = 16}, function(value)
   ws=value
end)
...
Section2:CreateToggle("Ile ESP", {Toggled=Settings.IslandE,Description = false}, function(state)
    Settings.IslandE=state
end)
local Tab2 = Window:CreatePage("Web Hook")
local Section2 = Tab2:CreateSection("Main")

Section2:CreateTextbox("Web Hook Url", "WebHook URL ici", function(args)
...
Section2:CreateToggle("DF Webhook", {Toggled=Settings.DFWebHook,Description = "Vous notifira en cas de drop de DF"}, function(state)
    ...
    function()
        while wait(1) do
            if Settings.AutoBuso and data.Stats.BusoMastery.Value > 0 then
                local a = true
                if Settings.BusoMethod == "Actif Mob Proximité" then
